...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.1">
    ...
    <xsl:template match="DomainDictionary">
        <xsl:variable name="list" select="//Entry"/>
        <xsl:call-template name="Entry">
            <xsl:with-param name="entries" select="$list"/>
        </xsl:call-template>
    </xsl:template>
    <!-- =========================================================== -->
    <xsl:template name="Entry">
        ...
        <xsl:if test="count($entries)&gt;=1">
            <xsl:variable name="firstEntry" select="$entries[1]"/>
            <!--<xsl:value-of select="$firstEntry/Name"/>-->
            <xsl:call-template name="Entry">
                <xsl:with-param name="entries" select="$entries[1]/following-sibling::*"/>
            </xsl:call-template>
        </xsl:if>
<!--        <xsl:variable name="docName">
              <xsl:call-template name="str:subst">
                <xsl:with-param name="text" select="normalize-space($entries[1]/Name)"/>
                <xsl:with-param name="replace"> </xsl:with-param>
                <xsl:with-param name="with">x</xsl:with-param>
              </xsl:call-template>        
        </xsl:variable>
        <xsl:document href="{concat('domainDictionary',$docName,'.html')}" method="xml">
-->
        <xsl:document href="{concat('domainDictionary',translate(normalize-space($entries[1]/Name),' ',''),'.html')}" method="html">
                    <html>
            <head>
            ...
            <body>
                ...
                <div id="content">
                            ...
                            <xsl:call-template name="DesignPattern">
                                <xsl:with-param name="entry" select="$entries[1]"/>
                            </xsl:call-template>    
                  </div>
    ...
    <!-- =========================================================== -->
    <xsl:template name="DesignPattern">
        <xsl:param name="entry"/>
        <p class="links">
            See the
            <a>
             <xsl:attribute name="HREF"><xsl:value-of select="$entry/DesignPattern/FileName"/>.html</xsl:attribute><xsl:value-of select="$entry/DesignPattern/Name"/>
            </a> design pattern.
        </p>     
       </xsl:template>
