...
TestCaseDummyDataItemControlBlock_1::TestCaseDummyDataItemControlBlock_1(void) :
	TestCaseWithEvtCheck(ID_DUMMYDATAITEMCONTROLBLOCK*10+1,"TestCaseDummyDataItemControlBlock_1") {
	return;
}

void TestCaseDummyDataItemControlBlock_1::runTestCase(void) {
	...
	DC_DummyDataItemControlBlock *pDCB         =new DC_DummyDataItemControlBlock;
    ...
    for (int i=0;i<numInputs;i++)
        pInputs[i]=new DC_DataItem(&inputs[i]);

    for (int i=0;i<numOutputs;i++)
        pOutputs[i]=new DC_DataItem(&outputs[i]);

	// Verify the correctness of the class identifier
    if (pDCB->getClassId()!=ID_DUMMYDATAITEMCONTROLBLOCK) {
    ...
    for (unsigned int i=0;i<numOutputs;i++) {
        ...
        pDCB->setOutputLink(i,pOutputs[i]);
    }
    ...
    if (isNonNominalCheckAllowed()) {
        ...
        pDCB->setInputLink(numInputs,pInputs[0]);
	    if (!verifyLatestEvent(nEvt+1,EVT_ILLEGAL_CB))
		    return;

        pDCB->setOutputLink(numOutputs,pOutputs[0]);
