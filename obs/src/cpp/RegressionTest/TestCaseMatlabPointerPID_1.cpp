...
TestCaseMatlabPointerPID_1::TestCaseMatlabPointerPID_1(void) :
	TestCaseGenericSetUp(ID_MATLABPOINTERPID*10+1,"TestCaseMatlabPointerPID_1") {
	return;
}

void TestCaseMatlabPointerPID_1::runTestCase(void) {
    TD_Float             input;
    TD_Float             output;
	const unsigned int   numParams=3;
    DC_MatlabPointerPID *pMPID    =new DC_MatlabPointerPID;

	// Verify correctness of class identifier
    if (pMPID->getClassId()!=ID_MATLABPOINTERPID) {
		setTestResult(TEST_FAILURE,"Wrong class identifier");
		return;
	}

	// A newly created DC_MatlabPointerPID shall not be configured
	...
	// (see pid.c and DC_MatlabPointerPID.cpp) are, respectively, -1.0, 1.0
	...
	// values (see pid.c and DC_MatlabPointerPID.cpp).
    input=(TD_Float)1.0;
