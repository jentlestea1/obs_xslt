...
DC_PUSDumpMemoryOffset::DC_PUSDumpMemoryOffset(void) {
    setClassId(ID_PUSDUMPMEMORYOFFSET);
    setSubType(PUS_ST_TC_DMP_OFF);
}

TD_ActionOutcome DC_PUSDumpMemoryOffset::doAction(void) {
    ...
    unsigned int packetLength = fixedLength+dataLength+numberBlocks*
                                    (sizeof(TD_PUSMemOffset)+sizeof(TD_PUSMemLength)+sizeof(unsigned short));

    // performa a consistency check: the number of blocks must be consistent with the
    // length of the application data
    if ( numberData!=(fixedLength+numberBlocks*(sizeof(TD_PUSMemOffset)+sizeof(TD_PUSMemLength))) )
    ...
    TD_PUSMemData* base;
    memcpy(&base,data+offset,sizeof(TD_PUSMemData*));
    pPck->setBaseAddress(base);
    offset = sizeof(TD_PUSMemId)+sizeof(TD_PUSMemData*)+sizeof(TD_PUSNumberMemBlocks);
    for (TD_PUSNumberMemBlocks i=0; i<numberBlocks; i++) {
        TD_PUSMemOffset blockOffset;
        memcpy(&blockOffset,data+offset,sizeof(TD_PUSMemOffset));
