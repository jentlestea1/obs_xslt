...
<xsl:text>
   </xsl:text>// ---- Configure the control  action "<xsl:value-of select="$contAct"/>" ----
<xsl:text/>
           <!-- Set the initial enable status -->
           <xsl:if test="InitialEnableStatus">
                 <xsl:choose>
                       <xsl:when test="@value='ENABLED'">
                           <xsl:text>   </xsl:text><xsl:value-of select="$contAct"/>->setInitialEnabledStatus(true);
<xsl:text/>
                       </xsl:when>
                       <xsl:when test="@value='DISABLED'">
                           <xsl:text>   </xsl:text><xsl:value-of select="$contAct"/>->setInitialEnabledStatus(false);
<xsl:text/>
                       </xsl:when>
                       <xsl:otherwise>
