...
<xsl:text/>
            ...
            <xsl:for-each select="Parameter">
               ...
               <xsl:text>   </xsl:text><xsl:value-of select="$contBlk"/>->setParameter(<xsl:value-of 
                           select="Number/@value"/>,CC_RootObject::getParameterDatabase()->getParameter<xsl:value-of select="$normalizedType"/>(<xsl:value-of select="DatabaseName/@value"/>));
<xsl:text/>
            </xsl:for-each>
      ...
      <xsl:template name="dataPoolLink">
           ...
           <xsl:for-each select="Input">
               <xsl:text>   </xsl:text><xsl:value-of select="$contBlk"/>->setInputLink(<xsl:value-of select="Number/@value"/>,<xsl:value-of select="DatapoolName/@value"/>);
<xsl:text/>               
           </xsl:for-each>
           <xsl:for-each select="Output">
   <xsl:text>   </xsl:text><xsl:value-of select="$contBlk"/>->setOutputLink(<xsl:value-of select="Number/@value"/>,<xsl:value-of select="DatapoolName/@value"/>);
<xsl:text/>
           </xsl:for-each>
      </xsl:template>            
