...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.1">
   ...
   CC_RootObject::setEventRepository(<xsl:value-of select="$evtRep"/>);

   // Load the parameter database
   CC_RootObject::setParameterDatabase(<xsl:value-of select="$parDB"/>);
           
   // Load the datapool
   CC_RootObject::setDataPool(<xsl:value-of select="$DP"/>);

   // Load the tracer
   CC_RootObject::setTracer(<xsl:value-of select="$tracer"/>);
